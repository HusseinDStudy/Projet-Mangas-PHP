...
$requete = new DialogueBD();?>
    ...
    <head>
        ...
        <!--<script src="../js/ajaxConnexion.js"></script>-->
    </head>
    <body>
    ...
    <form action="ModificationManga.php" method="post" class="text-center">
        ...
        if(!isset($_POST['manga'])){
            ...
            try {
                $mesMangas = $requete->getTous();
            } catch (Exception $e) {
                $erreur = $e->getMessage();
            }
            foreach ($mesMangas as $ligne) {
                $code = $ligne['id_manga'];
                $titre = $ligne['titre'];
        ...
        /*if(isset($_POST['manga'])&&!isset($_POST['titre'])||!isset($_POST['genre'])||!isset($_POST['couverture'])||!isset($_POST['dessinateur'])||!isset($_POST['scenariste'])||!isset($_POST['prix'])||!isset($_POST['id'])){
            $MangaAfficher=$requete->getUnMangaParID($_POST['manga']);
        ...
        <input type=\"number\" id=\"ID\" name=\"id\" size=\"3\" value=\"$id\" readonly><br>
        <label class=\"font-weight-bold\" for=\"Titre\">Titre: </label>
        <input id=\"Titre\" name=\"titre\" type=\"text\" placeholder=\"Entrer son titre\" size=\"50\" value=\" $titre\" required><br>
        ...
        <input id=\"Couverture\" name=\"couverture\" type=\"file\"  value=\"$couverture\" required><br>";
            if (isset($couverture)) {
                $readorhide = "readonly";
            }
            if (isset($_POST['couverture'])) $readorhide = "hidden";
            echo "<input type=\"text\" value=\"$couverture\" $readorhide><br>
        ...
        <input id=\"Prix\" name=\"prix\" type=\"number\" placeholder=\"Entrer un prix\" size=\"5\" value=\"$prix\" required><br>";
        }
        ?>
